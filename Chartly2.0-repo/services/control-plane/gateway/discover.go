...
type discoverProfile struct {
	...
	Source      profileSourceLite `json:"source"`
	Mapping     map[string]string `json:"mapping"`
}

type profileSourceLite struct {
...
func handleDiscoverDataGov(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]any{"error": "discover_failed"})
		return
	}
...
func fetchDataGov(q string, rows int) (discoverResp, error) {
	...
	for _, ds := range raw.Result.Results {
		...
		p := discoverProfile{
			...
			Source:      profileSourceLite{Type: "http_rest", URL: best, Auth: "none"},
