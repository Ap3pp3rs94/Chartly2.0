...
level = alpha*(train[i]-prevSeason) + (1-alpha)*(level+trend)


trend = beta*(level-prevLevel) + (1-beta)*trend


season[si] = gamma*(train[i]-level) + (1-gamma)*prevSeason
...
out[i-1] = level + float64(i)*trend + season[si]
...
// Time index regression (OLS) with optional ridge regularization (lambda>=0).
...
iv := time.Duration(cfg.IntervalSeconds) * time.Second
