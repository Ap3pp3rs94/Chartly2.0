...
// Backend implementations (redis, postgres, inmem)
// live elsewhere.
//
// Guidance:
...
type Producer interface {
	Enqueue(ctx context.Context, q QueueName, env Envelope)
	// error

	// EnqueueBatch publishes many envelopes. Implementations SHOULD enforce MaxBatchSize.
	EnqueueBatch(ctx context.Context, q QueueName, envs []Envelope)
	// error
}
...
type Consumer interface {
	...
	Ack(ctx context.Context, q QueueName, receipt string)
	// error

	// Nack returns a leased message to the queue; delay controls re-visibility.
	Nack(ctx context.Context, q QueueName, receipt string, delay time.Duration)
	// error
	...
	NackWithDeadLetter(ctx context.Context, q QueueName, receipt string, delay time.Duration, reason string)
	// error

	// ExtendVisibility extends the lease on a leased message.
	ExtendVisibility(ctx context.Context, q QueueName, receipt string, visibilityTimeout time.Duration)
	// error
}
...
type DeadLetter interface {
	MoveToDLQ(ctx context.Context, q QueueName, receipt string, reason string)
	// error
}
