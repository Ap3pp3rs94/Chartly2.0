...
if depth > maxDepth {
rep.addIssue(CSevError, "limits.depth", "", path, fmt.Sprintf("max depth exceeded (%d)
during encoding", maxDepth))
return write([]byte("null"))

}
if nodeBudget <= 0 {
rep.addIssue(CSevError, "limits.nodes", "", path, fmt.Sprintf("max nodes exceeded (%d)
during encoding", maxNodes))
return write([]byte("null"))
...
case map[string]any:
if len(x) > maxMapKeys {
rep.addIssue(CSevError, "limits.map_keys", "", path, fmt.Sprintf("map has too many keys (%d>%d)
during encoding", len(x), maxMapKeys))
return write([]byte("{}"))
...
rep.addIssue(CSevError, "limits.array_len", "", path, fmt.Sprintf("array too long (%d>%d)
during encoding", len(x), maxArrayLen))
return write([]byte("[]"))
