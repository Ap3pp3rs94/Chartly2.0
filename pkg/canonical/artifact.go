...
// - Hash chain (PrevHash/Hash) is a METADATA hash, not a content hash.
...
// - MetadataBytes(): includes mutable metadata (title/notes/attrs/links) for tamper-evidence chaining.
// - ComputeHash(prevHash): sets PrevHash + Hash over MetadataBytes().
//
// URI validation:
// - ValidateURI() is strict by default: allow https://, s3://, gs:// only.
// - ValidateURIWithPolicy() allows callers to opt-in to additional schemes (e.g., file:// for local).
...
// Includes mutable fields (Title/Notes/Attributes/Related refs) and PrevHash,
