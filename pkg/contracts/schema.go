...
//
// Security:
// - root is pinned (EvalSymlinks) and reads are prevented from escaping it.
...
if err := write([]byte("{")); err != nil { return err }
for i, k := range keys {
if i > 0 { if err := write([]byte(",")); err != nil { return err } }
ks, err := json.Marshal(k)
if err != nil { return err }
if err := write(ks); err != nil { return err }
if err := write([]byte(":")); err != nil { return err }
if err := enc(path+"."+k, x[k], depth+1); err != nil { return err }
...
for i := 0; i < len(x); i++ {
if i > 0 { if err := write([]byte(",")); err != nil { return err } }
if err := enc(fmt.Sprintf("%s[%d]", path, i), x[i], depth+1); err != nil { return err }
